<template>
  <el-form ref="formRef" :model="props.formData" :rules="props.formRules" v-bind="$attrs">
    <el-row>
      <el-col v-for="(column, index) in props.formColumns" :key="index" :span="column.span" :offset="column.offset">
        <template v-if="column.slotName">
          <slot :name="column.slotName"></slot>
        </template>
        <el-form-item :label="column.label" :prop="column.prop" v-else v-bind="column.formItemOpts">
          <component :is="componentsTypes[column.xType]" v-bind="column" v-model="props.formData[column.prop]">
          </component>
        </el-form-item>
      </el-col>
    </el-row>
